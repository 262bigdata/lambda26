...
4. Funciones, agrupaciones y agregaciones.
5. RDD: el modelo debajo de los DataFrames.
...
Con `schema=schema` explícito, Spark ya no necesita `inferSchema=True` (se ahorra la pasada extra de lectura) y `codigo` conserva el cero inicial porque tú declaraste `StringType()`, no porque Spark lo haya adivinado bien. No es la única forma de resolverlo — en 2.6 lo arreglamos de otra manera, con `.cast("string")` después de leer con `inferSchema=True`; ambas son válidas, y cuál conviene depende de si prefieres controlar el tipo desde la lectura o corregirlo después. En 3.6 vas a encontrar este mismo problema con datos reales del dataset H&M.
...
`groupBy()` también acepta más de una columna a la vez — por ejemplo, `df.groupBy("cliente_id", "canal").agg(...)` agrupa por la combinación de ambas, no por cada una por separado. En 3.7 vas a aplicar exactamente este patrón sobre transacciones reales del dataset H&M — con una advertencia importante sobre qué significa realmente la columna que vas a sumar.
...
| Ejemplo en esta sesión | `groupBy().agg()` sobre datos tabulares (aplicado en 3.7 sobre transacciones reales de H&M). | Conteo de palabras sobre texto libre (aplicado en 3.8 sobre descripciones reales de producto). |
...
- **3.3** Cargar y explorar `customers.csv`.
- **3.4** Aplicar transformaciones y verificar la evaluación perezosa.
- **3.5** Analizar el plan de ejecución con `explain()`.
- **3.6** Aplicar funciones y crear columnas con `withColumn()`.
- **3.7** Aplicar agrupaciones y agregaciones (`transactions.parquet`).
- **3.8** Convertir a RDD y procesar texto (`detail_desc` de `articles.csv`).
- **3.9** Documentar hallazgos y responder preguntas de reflexión.
...
**Producto del paso:** DataFrame `df_customers` cargado, estructura y primeras filas verificadas.

```python
df_customers = spark.read.csv(
    "/opt/data/hm/Origen/customers.csv",
    header=True,
    inferSchema=True,
)
...
print(df_customers.columns)  # lista con los nombres de columna

num_rows, num_cols = df_customers.count(), len(df_customers.columns)
print(f"Filas: {num_rows}, Columnas: {num_cols}")
```

Si quieres evitar por completo la ambigüedad de `inferSchema` (ver 2.4), puedes declarar un `StructType` para `customers.csv` siguiendo el mismo patrón que viste en la teoría (aplicado ahí a `articles.csv`) — es opcional en este paso, no obligatorio.
...
**Producto del paso:** `articles.csv` cargado, con al menos tres columnas nuevas o corregidas mediante `withColumn()`.

Aplica el mismo patrón de 2.6, ahora sobre datos reales — cada línea corrige o crea una columna con una función distinta:

```python
from pyspark.sql.functions import col, when, lit, current_date

df_articles = spark.read.csv("/opt/data/hm/Origen/articles.csv", header=True, inferSchema=True)

df_articles = df_articles.withColumn("article_id", col("article_id").cast("string"))
# .cast() de 2.6, corrigiendo el mismo problema de 2.4: article_id son solo dígitos,
# inferSchema le quita el cero inicial sin avisar.
...
# when()/otherwise() de 2.6, aplicado a la columna real perceived_colour_value_name
# (valores reales: "Dark", "Light", u otros).

df_articles = df_articles.withColumn("fuente", lit("H&M Kaggle"))        # lit() de 2.6: columna constante
df_articles = df_articles.withColumn("fecha_procesado", current_date())  # current_date() de 2.6: la calcula Spark, no tú
...
```python
df_transactions = spark.read.parquet("/opt/data/hm/Origen/transactions.parquet").limit(100000)

df_transactions.printSchema()  # confirma tú mismo las columnas: no asumas el esquema (ver 2.4)
```

Corrige los tipos antes de agrupar — el mismo patrón encadenado de 2.6, aplicado ahora a las columnas reales de `transactions.parquet`:
...
df_transactions = (
    ...
    # to_date() de 2.6: t_dat llega como texto ("2020-01-15"), no como fecha.
    .withColumn("article_id", col("article_id").cast("string"))
    # mismo riesgo de cero inicial que en 2.4/3.6 — article_id se repite en esta tabla,
    # porque transactions.parquet referencia el mismo article_id de articles.csv.
    .withColumn("price", col("price").cast("double"))
)
```

**Segunda forma de leer el mismo archivo:** en vez de corregir los tipos *después* con `withColumn()`, puedes declarar el esquema *desde la lectura* — el mismo patrón de `StructType` de 2.4, aplicado ahora a `transactions.parquet`. Es la misma técnica que vas a necesitar más adelante (S8) para leer datos en tiempo real: Spark Structured Streaming no admite `inferSchema`, exige un esquema explícito desde el inicio.

```python
from pyspark.sql.types import StructType, StructField, StringType, DoubleType, IntegerType

schema_transactions = StructType([
    StructField("t_dat", StringType(), nullable=True),
...
df_transactions = (
    ...
    .parquet("/opt/data/hm/Origen/transactions.parquet")
    .limit(100000)
)
```

Con este esquema, `customer_id` y `article_id` ya llegan como texto (sin el riesgo de perder ceros iniciales) directamente desde la lectura — no necesitas el `.cast("string")` de la celda anterior para esas dos columnas. `t_dat` sigue llegando como texto: convertirlo a fecha real todavía requiere `to_date()`, porque Parquet no aplica un formato de fecha solo por declarar el tipo en el esquema — ese paso de la celda anterior sigue siendo necesario.
...
Aplica el patrón RDD de 2.8 sobre `detail_desc` — las ~105 000 descripciones de producto reales de `articles.csv`:
...
Agrega celdas markdown breves debajo de cada bloque de código (3.3-3.8) explicando qué hiciste y qué observaste — es la base directa de la evidencia técnica que armarás en 4.3.1.
...
4. Aplicar al menos una función (`withColumn`, `when`, `lit` o `.cast()`) sobre una columna del caso del equipo (equivalente a 3.6).
5. Aplicar una agrupación con al menos una función de agregación relevante al caso del equipo (equivalente a 3.7).
6. Convertir a RDD y aplicar al menos dos operaciones tipo `map`/`flatMap`/`filter`/`reduceByKey` (equivalente a 3.8).
...
2. *Transformaciones, funciones y evaluación perezosa*
    - Transformaciones y funciones aplicadas, con evidencia de que la ejecución ocurre recién con la acción (equivalente a 3.4 y 3.6).
3. *Agrupaciones y agregaciones*
    - Resumen agregado relevante al caso del equipo (equivalente a 3.7).
4. *RDD y plan de ejecución*
    - Procesamiento RDD y plan de `explain()` interpretado (equivalente a 3.5 y 3.8).
